...
<body>
    ...
    <div class="main-blog">
        <div class="blog-heading d-flex justify-content-between align-items-center mt-5 mb-5 ">
            ...
            <form action="{{ route('save-blog', $blog->id) }}" method="POST">
                @csrf
                <button class="btn btn-outline-success me-5 pe-5">Add to favourite</button>
            </form>
        </div>


        <div class="blog-writer m-5 ">
            <h6 class="me-5 pe-5">Writer: {{ $blog->writer->name ?? "Unknown" }}</h6>
        </div>
        ...
        <div class="blog-image d-flex justify-content-center">
            <img src="{{ asset('storage/' . $blog->image) ?? 'No image available' }}" alt="" height="400"
                width="60%">
        </div>
